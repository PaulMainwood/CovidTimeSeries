...
#a best-fit line.
proportions_new_variant <- function(new_variant_name, base_variant_name, grp, aggregate_by_week_threshold = 2, aggregate_by_week_days = 21) {
  ...
  #Aggregate up by week if less than
  if (sum(tail(df$new, aggregate_by_week_days), na.rm = T) / aggregate_by_week_days < aggregate_by_week_threshold){
    df <- df %>% group_by(sample_date = cut(sample_date, "week")) %>% summarise(base = sum(base, na.rm =T), new = sum(new, na.rm = T))
  }
  ...
  df <- mutate(df, wts = wts/sum(wts) * dim(df)[1])
...
base_variant = "BA.5.2"
new_variants = c("BA.4.6", "BA.5.1", "BA.2.75", "BE.1", "BF.3")
dfs <- sapply(new_variants, function(x) proportions_new_variant(x, base_variant, grp, aggregate_by_week_threshold = 2))
df <- bind_rows(dfs[1, ])
...
ggplot(df, aes(x = sample_date, y = new_frac_log2, color = new_variant_name)) + 
  geom_point(size = 3, aes(alpha = wts)) + 
