...
plot(date(df$sample_date), log2(df$ba4_frac), pch = 19, col = 2, ylim = c(-13, 0), xlim = c(first_date, last_date), yaxt = "n", xaxt = "n", ylab = "Ratio to BA.2, % - log2 scale", xlab = "Date", main = "BA.4,BA.5 & BA.2.12.1 as a ratio to BA.2 in COG-UK sequences")
...
legend("bottomright", inset = 0.05, legend = c("BA.4", "BA.5", "BA.2.12.1"), col = c(2, 3, 4), lty = c(1, 1))
title(sub="Plot of COG-UK data, by @paulmainwood, based on previous work from @alexselby1770. Regression lines weighted by sample size; last few data points based on very small values", adj=1, line=4, font=1, cex.sub = 0.6)
